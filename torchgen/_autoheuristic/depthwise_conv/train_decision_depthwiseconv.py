...
class TrainDecisionTreeDepthwiseConv:
    ...
    def constant_subtree_classes(self, tree):
        """Map each node to its class if its whole subtree predicts one class.

        sklearn keeps splits that lower weighted impurity even when both sides
        share the same argmax class, so the raw tree contains branches where
        every reachable leaf returns the same label. Collapsing such a subtree
        to a single leaf is decision-preserving and removes those dead branches
        ...
        def visit(node_id):
            if self.is_leaf_node(tree, node_id):
                constant[node_id] = self.get_leaf_class(tree, node_id)
            else:
                left = visit(tree.children_left[node_id])
                right = visit(tree.children_right[node_id])
                constant[node_id] = left if left is not None and left == right else None
            return constant[node_id]
...
"""
    ...
    def codegen(self, tree, lines: list[str]):
        ...
        constant = self.constant_subtree_classes(tree)

        def codegen_node(node_id, depth):
            """Recursively traverse tree nodes and generate C++ code."""
            indent = "  " * (depth + 1)

            # Collapse any subtree (including a plain leaf) that predicts a
            # single class. This is what prunes the dead branches sklearn emits.
            if constant[node_id] is not None:
                lines.append(f"{indent}return {constant[node_id]};")
                return
            ...
            # A child that maps to a constant class is emitted inline as a
            # single return. The node itself is not constant here, so the two
            # children cannot both be the same constant class.
            left_class = constant[left_child]
            right_class = constant[right_child]
